...
2. `2026-08-16-ai-router-and-candidate-profile.md` — 9Router gateway, bounded retry/circuit handling, schema-valid candidate profile extraction, review/edit/active CV.
...
- AI: configurable 9Router OpenAI-compatible gateway; every structured response is validated by Pydantic.
- 9Router: server-side `NINEROUTER_API_KEY` and configured `NINEROUTER_MODEL`; model identifiers are configuration, not hardcoded defaults.
- Semantic similarity: optional 9Router embedding model when configured, with deterministic lexical degradation when embeddings are unavailable.
