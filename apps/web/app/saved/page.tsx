...
export default async function SavedJobsPage() {
  ...
  const { data: rows } = await supabase
    .from("user_jobs")
    .select("job_id, jobs(title, company, location, work_mode, employment_type, published_at, source_name, original_url)")
    .eq("user_id", user.id)
    .eq("status", "saved")
    .order("updated_at", { ascending: false });

  const saved = (rows as SavedJob[] | null) ?? [];
              ...
              <li key={item.job_id} className="rounded-2xl border border-[#d9d5cc] bg-white p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    <JobActions jobId={item.job_id} status="saved" />
                  </div>
                </div>
              </li>
            );
